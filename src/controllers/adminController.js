...
const rechargeDuyet = async (req, res) => {
    ...
    if (type == 'confirm') {
        ...
        await connection.query('UPDATE users SET money = money + ?, total_money = total_money + ? WHERE phone = ? ', [info[0].money + (info[0].money / 100) * 5, info[0].money + (info[0].money / 100) * 5, info[0].phone]);
        return res.status(200).json({
